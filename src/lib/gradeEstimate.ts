...
export function calculateGradeEstimate(input: {
  ...
  const scaledScore = Math.round(percent * input.boundary.totalMarks);

  const grade = gradeOrder.find((value) => scaledScore >= input.boundary.boundaries[value]) ?? "1";
  const currentBoundary = input.boundary.boundaries[grade];
  const nextGrade = gradeOrder[gradeOrder.indexOf(grade) - 1] ?? null;
  const nextBoundary = nextGrade ? input.boundary.boundaries[nextGrade] : null;
