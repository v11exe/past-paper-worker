...
export function buildPaperProcessingPrompt(input: {
  ...
  return [
    ...
    "5. Only convert simple formats: simple checkbox/radio, simple multi-checkbox, short answer, long answer, and calculations. AQA phrases such as Tick one box, Shade one lozenge, Choose one answer are single_choice; Tick two boxes or Choose two answers are multi_select when the options are visible.",
    "5a. If the source is a table/grid/matrix, row-by-row checkbox table, matching table, drawing, plotting, shading, diagram-completion, or diagram-labelling task, set originalContent.unsupportedQuestionFormat true and set originalContent.unsupportedReason. Never pretend those formats are ordinary text boxes.",
