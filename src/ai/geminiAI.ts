...
import { hasUnredactedGoogleApiKey, redactSensitiveText, redactSensitiveValue } from "./redaction";

type AITestMock = {
  respond?: (request: AIProxyRequest) => Promise<AIProxyResponse> | AIProxyResponse;
...
export const FALLBACK_AI_MODELS = ["gemini-2.5-flash"] as const;
export const AI_MODEL_CHOICES = [DEFAULT_AI_MODEL, ...FALLBACK_AI_MODELS] as const;
...
class AIRequestTimeoutError extends Error {
  constructor(timeoutMs: number) {
    super(`Gemini request timed out after ${Math.round(timeoutMs / 1000)}s`);
...
function logSchemaFailure(input: { label: string; raw: string; extractedJson: string; parsed: unknown; normalized: unknown; error: z.ZodError }) {
  console.error(`[Gemini AI] ${input.label} schema validation failed`);
  console.error("[Gemini AI] Exact raw response:", redactSensitiveText(input.raw));
  console.error("[Gemini AI] Exact extracted JSON:", redactSensitiveText(input.extractedJson));
  console.error("[Gemini AI] Parsed JSON before normalization:", preview(input.parsed, 5000));
  if (input.normalized !== input.parsed) {
    console.error("[Gemini AI] Parsed JSON after normalization:", preview(input.normalized, 5000));
  }
  console.error("[Gemini AI] Exact schema failure reason:", formatSchemaIssues(input.error));
  console.error("[Gemini AI] Full Zod issues:", preview(input.error.issues, 5000));
}
...
function parseDataUrl(dataUrl: string) {
  ...
  if (!match) {
    throw new Error("Only base64 data URLs can be sent to the Gemini proxy.");
  }
...
async function callTestMock(request: AIProxyRequest) {
  ...
  if (!mock) return null;
  if (mock.respond) return aiProxyResponseSchema.parse(await mock.respond(request));
...
async function callProxy(request: AIProxyRequest) {
  ...
  try {
    ...
    throw new Error(`Gemini proxy returned invalid JSON (${response.status}). ${preview(raw)}`);
  }
  const normalized = aiProxyResponseSchema.safeParse(parsed);
  if (!normalized.success) {
    throw new Error(`Gemini proxy returned an unexpected payload (${response.status}).`);
  }
...
export async function aiStructuredJson<S extends z.ZodTypeAny>(prompt: string, schema: S, options: StructuredJsonOptions): Promise<z.output<S>> {
  ...
  if (!result.success) {
    ...
    options.onSchemaError?.({
      label,
      paths: result.error.issues.map(issuePath),
    ...
    diagnosticsRedactionCheck: {
      ...
      redacted:
        typeof redactionPreview === "string"
          ? !hasUnredactedGoogleApiKey(redactionPreview) && /\[REDACTED/.test(redactionPreview)
          : null,
    },
  };
}
